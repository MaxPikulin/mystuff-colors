...
//TODO add search ability to filter divs realtime(like on google search bar).
...
function searchBar(e) {
    ...
    divs.forEach(function(v) {
        if (v.dataset.name.includes(val)) {
            //v.style.position = 'relative';
            //v.style.transform = 'unset';
            v.style.display = 'inline';
        } else {
            //v.style.transform = 'translateX(-400px)';
...
for (var i = 0, n = color.length; i < n; i++) {
    ...
    div += `<div data-name="${name.toLowerCase()}" style="background-color:${num};"><button style="color: ${num};" title="${num}"><textarea>${num}</textarea>${name}</button></div>`;
}
colorsDiv.innerHTML += div;
const divs = colorsDiv.querySelectorAll('div');
/*divs.forEach(d => {
    const [r, g, b] = d.style.backgroundColor.match(/\d+/g);
...
colorsDiv.addEventListener('click', function(e) {
    if (e.target.localName == 'button') {
        const copy = e.target.firstChild.select();
